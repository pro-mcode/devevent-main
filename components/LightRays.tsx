...
/* Uniform value types: use tuples for vector uniforms */
type Vec2 = [number, number];
type Vec3 = [number, number, number];

type UniformValue = number | Vec2 | Vec3;

type Uniform<T extends UniformValue = UniformValue> = {
  value: T;
};
...
const LightRays: React.FC<LightRaysProps> = ({
  ...
  /* Intersection observer to only init when visible */
  useEffect(() => {
  ...
  /* Initialize / teardown WebGL when visible */
  useEffect(() => {
    if (!isVisible || !containerRef.current) return;

    // Cleanup if previous exists
    ...
    const initializeWebGL = async () => {
      ...
      // tiny delay to ensure DOM size is stable
      ...
      /* shaders (kept the same as your original) */
      const vert = `
...
void main() {
  ...
  gl_FragColor  = color;
      ...
      const updatePlacement = () => {
        ...
        rendererLocal.dpr = Math.min(window.devicePixelRatio || 1, 2);
      ...
      const loop = (t: number) => {
        ...
        if (followMouse && mouseInfluence > 0) {
          ...
          smoothMouseRef.current = [
            smoothMouseRef.current[0] * smoothing +
              mouseRef.current[0] * (1 - smoothing),
            smoothMouseRef.current[1] * smoothing +
              mouseRef.current[1] * (1 - smoothing),
          ];
          uniformsRef.current.mousePos.value = smoothMouseRef.current;
        }
        ...
        try {
          ...
          // if WebGL context lost or rendering error, bail gracefully
      ...
      cleanupRef.current = () => {
        ...
        try {
          if (rendererRef.current) {
            const canvas = rendererRef.current.gl
          ...
          console.warn("Error while cleaning up WebGL:", e);
        }
